...
$("#peepso-wrap .ps-privacy-dropdown ul li a").click(function(e) {
	var a = $(e.target).closest("a"),
		...
		placeHolder = btn.find("span.ps-privacy-title");
...
function initDatepicker( $dp ) {
	...
	if ( window.peepsodatepickerdata && peepsodatepickerdata.config ) {
		...
		for ( var key in daysShort ) {
			daysMin.push( daysShort[key].replace(/^([a-z]{2}).+$/i, '$1') );
		}
...
$(function() {
	initDatepicker( $('.datepicker').not('.datepicker-initialized') );
});
