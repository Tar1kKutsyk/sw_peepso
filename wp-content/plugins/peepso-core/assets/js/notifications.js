...
(function($){
	function PsPopoverNotification(elem, options)
	{
		...
		this.load_page = function(callback) {
			if (false === this._content_is_fetched) {
				var loadingClass = 'ps-popover-loading',
					loading = this.popover_list.nextAll('.' + loadingClass);
	...
	function get_latest_count() {
		if (peepsodata.currentuserid > 0) {
			$PeepSo.disableAuth().disableError().postJson("notificationsajax.get_latest_count", null, function(response) {
