...
//	private $current_user = 0;				// used in modifying query
    ...
    public $template_tags = array(
        ...
        'show_commentsbox_addons', // output for commentsbox addons
    );
...
//PeepSo::log('  act post data: ' . var_export($aActData, TRUE));
		...
		$rank = new PeepSoActivityRanking();
		$rank->add_comment_count($act_data->act_id);
...
//PeepSo::log(__METHOD__.'() setting comment_query to NULL');
    ...
    {
        $input = new PeepSoInput();
        $this->first_post_id = $input->post_int('first', NULL);

        $activity = $this->get_activity($input->post_int('act_id'));
    ...
    public function show_more_comments_link()
    {
        ...
        if ($this->comment_query->max_num_pages > 1)
            echo sprintf(__('Show %1$d more comments', 'peepso'), $this->get_number_of_more_comments() );
...
//				PeepSoMailQueue::add($owner_id, $data, __('Someone Liked your post', 'peepso'), 'likepost');
		...
		$data = array(
                    'permalink' => PeepSo::get_page('activity') . '?status/' . $orig_post->post_title . ($activity_type == 'comment' ? '#comment-item-' . $post_id : ''),
                    'post_content' => $orig_post->post_content,
                );
                ...
                PeepSoMailQueue::add_message($owner_id, $data, sprintf(__('Someone Liked your %s', 'peepso'), $activity_type), 'like_' . $activity_type, 'like_' . $activity_type, PeepSoActivity::MODULE_ID);
    ...
    public function blockuser(PeepSoAjaxResponse $resp)
    {
        ...
        if (PeepSo::check_permissions($user_id, PeepSo::PERM_POST, $user_id)) {
            ...
            if ($res) {
                ...
                $resp->set('message', sprintf(__('The user %1$s has been blocked', 'peepso'), $user->get_fullname()));
    ...
    public function unblockuser(PeepSoAjaxResponse $resp)
    {
        ...
        if (PeepSo::check_permissions($user_id, PeepSo::PERM_POST, $user_id)) {
            ...
            if ($res) {
                ...
                $user = new PeepSoUser($block_id);
                $resp->set('header', __('Notice', 'peepso'));
                $resp->set('message', sprintf(__('The user %s has been unblocked', 'peepso'), $user->get_fullname()));
                $resp->set('actions', $actions);
            }
        }
    ...
    public static function set_ban_status(PeepSoAjaxResponse $resp)
    {
        ...
        if( 0 === $ban_status ) {
            ...
            $resp->set('message', sprintf(__('%s has been unbanned', 'peepso'), $user->get_fullname()));
            $resp->success(1);
        } else {
            $ban_type = $input->post('ban_type', 'ban_forever');
            if($ban_type == 'ban_period') {
                $ban_period_date = $input->post('ban_period_date');
                if(!empty($ban_period_date)) {
                    ...
                    $resp->set('message', sprintf(
                        __('%s has been banned until %s', 'peepso'),
                        $user->get_fullname(),
                ...
                {
                    $resp->success(0);
                    $resp->error(__('Missing ban period date', 'peepso'));
                    return;
                }

            } else {
                $user->set_user_role('ban');
                $resp->set('message', sprintf(__('%s has been banned', 'peepso'), $user->get_fullname()));
                $resp->success(1);
            }
        }
    }
...
//				'type' => $type,
        ...
        if (PeepSo::get_user_id() === intval($post->author_id) || PeepSo::get_user_id() === intval($post->act_owner_id) || PeepSo::is_admin()) {
            ...
            if (('0' !== $show_preview) && (!empty($media)) && ($post->act_module_id == PeepSoActivity::MODULE_ID))
                $acts['remove_link_preview'] = array(
                    'href' => '#remove-preview',
                    'label' => __('Remove Link Preview', 'peepso'),
                    'class' => 'actaction-removepreview',
                    'icon' => 'eye-off icon-eye-off', // 'trash',
    ...
    /**
     * Outputs link for the current post
     */
    public function link($post = NULL, $echo = FALSE)
    {
    ...
    public function post_actions()
    {
        ...
        if (! PeepSo::get_option('site_repost_enable', TRUE) ||
            !$logged_in || intval($post->post_author) === PeepSo::get_user_id() || (!in_array($post->act_access, array(PeepSo::ACCESS_MEMBERS, PeepSo::ACCESS_PUBLIC)))) {
            unset($acts['repost']);
        }
...
//PeepSo::log(__METHOD__.'() - post data: id=' . $this->post_data['ID']); // var_export($this->post_data, TRUE));
    ...
    public function get_like_names(PeepSoAjaxResponse $resp)
    {
        ...
        if (count($names) > 0) {
            ...
            $html .= _n(' likes this', ' like this', 1, 'peepso');
    ...
    public function ajax_show_post(PeepSoAjaxResponse $resp)
    {
        ...
        $owner_id = $input->post_int('user_id');
        $user_id = $input->post_int('uid');
        $act_id = $input->post_int('act_id');
    ...
    public function after_post_author()
    {
        ...
        if ($repost) {
            ...
            printf(__('via %s', 'peepso'),
                '<a href="' . $author->get_profileurl() . '">' . $author->get_fullname() . '</a>');
        }
    }
    ...
    public function post_action_title()
    {
        ...
        if ($post->post_author === $post->act_owner_id) {
            ...
            $title = sprintf('<a class="ps-stream-user" href="%s">%s</a> <span class="ps-stream-action-title">' . $action . '</span> ',
                $user->get_profileurl(), $user->get_fullname());
        } else {
            ...
            $title = sprintf(
                ...
                $author->get_profileurl(), $author->get_fullname(),
                $owner->get_profileurl(), $owner->get_fullname()
            );
        }
