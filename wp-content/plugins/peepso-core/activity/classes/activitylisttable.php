...
//			return ('<a href="' . PeepSo::get_page('activity') . '?status/' . $item->$column_name . '/" target="_blank">' . $item->$column_name . '</a>');
		...
		case 'user_avatar':
			$content = '<a href="' . $user->get_profileurl() . '" target="_blank" title="' . $user->get_fullname() . '">';
			$content .= '<img src="' . $user->avatar . '" title="' . $user->get_fullname() . '" width="48" height="48" alt="" style="float:left;margin-right:3px;"/></a>';
			return ($content);
