...
class PeepSo
{
	...
	private function __construct()
	{
		...
		add_filter('peepso_mail_digest_content', array(&$this, 'mail_digest_attach_media'));
	}

	public function filter_default_stream_options( $options )
	{
		$options[1] = 'Community feed';
		ksort($options);
		return $options;
	}
	...
	function action_peepso_profile_completeness_redirect() {
		...
		if( 1 == get_user_meta( $this->get_user_id(), 'peepso_after_register_profile_complete', TRUE)) {
			return TRUE;
		}

		$user = new PeepSoUser($this->get_user_id());
	...
	public function opengraph_tags()
	{
		...
		switch ($post->post_name)
		{
			case PeepSo::get_option('page_activity') :
				...
				add_filter('filter_activity_permission', array(&$this, 'filter_activity_permission'));
				$post_slug = $url->get(2);
				if (!empty($post_slug))
				{
					$activity = apply_filters('filter_activity_permission', sanitize_key($post_slug));

					if ($activity->act_access == PeepSo::ACCESS_PUBLIC)
					{
						$user = new PeepSoUser($activity->post_author);

						$tags = apply_filters('peepso_filter_opengraph_' . $activity->act_module_id, $tags, $activity);
						$tags['title'] .= ' - Post by ' . $user->get_fullname();
			...
			case PeepSo::get_option('page_profile') :
				...
				if ($url->get(1))
				{
					...
					if (FALSE === $user) {
						$user = get_user_by('id', PeepSo::get_user_id());
					}
				} else
				{
					$user = get_user_by('id', PeepSo::get_user_id());
	...
	public function filter_activity_permission($permalink)
	{
		global $wpdb;
		...
		if ( !is_admin() ) {
			...
			if ( isset($post->post_content) && stristr($post->post_content, '[peepso')) {
				...
				if (isset($title['newtitle']) && $title['newtitle'] != '') {
					$this->wp_title = array('old_title' => $old_title, 'title' => $title['title'], 'newtitle' => str_replace('stream', __('stream', 'peepso'), $title['newtitle']));
				...
				if ('ban' == $user->get_user_role()) {
					...
					if(empty($ban_date)) {
						...
						if($suspense_expired > 0)
						{
	                        wp_logout();
							echo "<script type=text/javascript>"
								." alert('" . sprintf(__('Your account has been suspended until %s.', 'peepso') , date_i18n(get_option('date_format'), $ban_date)) ."');"
								. "window.location.replace('" . PeepSo::get_page('activity') . "');"
								. "</script>";
							die();
						} else {
...
//self::log("adding directory [{$dirname}] to path list");
	...
	public function activation_redirect()
	{
		...
		if (get_option('peepso_do_activation_redirect', FALSE) || isset($_GET['freshinstall']))  {
			set_transient('peepso_cache_set_welcome_screen',1,3600*24*365);
			...
			if(get_transient('peepso_cache_set_welcome_screen')) {
				delete_transient('peepso_cache_set_welcome_screen');
	...
	public function enqueue_scripts()
	{
		...
		$aData = array(
			...
			'profile_unsaved_notice' => __('There are unsaved changes on this page.', 'peepso'),
			'profile_saving_notice' => __('The system is currently saving your changes.', 'peepso'),
		);
...
//		wp_enqueue_script('peepso-login');
		...
		wp_localize_script('peepso-member', 'peepsomemberdata', array(
			'ban_popup_title' => __('Ban this user', 'peepso'),
			'ban_popup_content' => PeepSoTemplate::exec_template('profile', 'dialog-ban', array('start_date' => $ban_start_date), TRUE),
			'ban_popup_save' => __('Ban this user', 'peepso'),
			'ban_popup_cancel' => __('Cancel', 'peepso'),
		));
	...
	public function new_comment_author_profile_link($return, $author, $comment_ID){
	    ...
	    if( 1 === intval(PeepSo::get_option('always_link_to_peepso_profile', 0))) {
			$user = new PeepSoUser($comment->user_id);
			if($user){
		...
		if (!is_wp_error($user) && NULL !== $user) {
			...
			if ($ban) {
				...
				if(!empty($ban_date)) {
					...
					if($suspense_expired > 0)
					{
						return (new WP_Error('account_suspended', sprintf(__('Your account has been suspended until %s.' , 'peepso'), date_i18n(get_option('date_format'), $ban_date) )));
					}
					else
					{
						// unset ban_date
						// set user role to member
						$psuser->set_user_role('member');
	...
	*/
	public static function filter_cron_schedules($schedules)
	{
		...
		$schedules['one_minute'] = array(
			'interval' => 60,
			'display' => __('Every One Minute', 'peepso')
		);
	...
	public function mail_digest_attach_media($post)
	{
	    $peepso_media = get_post_meta($post->ID, 'peepso_media');

	    if (count($peepso_media) == 0)
		return $post;

	    $post->post_attachment = PeepSoTemplate::exec_template('general', 'mail-digest-media', array('image' => $peepso_media[0]['og_image']), TRUE);
	...
	function filter_user_roles(WP_User_Query $user_query)
	{
		...
		if ($user_query->query_vars['peepso_roles']){
