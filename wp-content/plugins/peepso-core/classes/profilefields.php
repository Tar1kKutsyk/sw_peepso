...
class PeepSoProfileFields
{
	...
	public static function install( $verbose = FALSE )
	{
		...
		$fields = array(
				'first_name' => array(
						'post' => array(
								'post_title'	=> __('First Name','peepso'),
								'post_content'	=> __('What\'s your name?','peepso'),
						),
						'meta'	=>	array(
								'order'						=>	1,
								'class'						=> 	'text',
				...
				'last_name' => array(
						'post' => array(
								'post_title'	=> __('Last Name','peepso'),
								'post_content'	=> __('What\'s your last name?','peepso'),
						),
						'meta'	=>	array(
								'order'						=>	2,
				...
				'gender' => array(
						'post' => array(
								'post_title'	=> __('Gender','peepso'),
								'post_content'	=> __('What\'s your gender?','peepso'),
						),
						'meta'	=>	array(
								'order'						=>	3,
								'class'						=> 	'selectsingle',
				...
				'birthdate' => array(
						'post' => array(
								'post_title'	=> __('Birthdate','peepso'),
								'post_content'	=> __('When were you born?','peepso'),
						),
						'meta'	=>	array(
								'order'						=>	4,
								'class'						=> 	'textdate',
				...
				'description' => array(
						'post' => array(
								'post_title'	=> __('About Me','peepso'),
								'post_content'	=> __('Tell us something about yourself.','peepso'),
						),
						'meta'	=>	array(
								'order'						=>	5,
								'class'						=> 	'text',
				...
				'user_url' => array(
						'post' => array(
								'post_title'	=> __('Website','peepso'),
								'post_content'	=> __('What\'s your website\'s address?','peepso'),
						),
						'meta'	=>	array(
								'order'						=>	6,
								'class'						=> 	'texturl',
	...
	public static function reset()
	{
		...
		if(isset($_GET['reset']) && 'all' == $_GET['reset']) {
	...
	public function load_fields( $args = array() )
	{
		...
		if($q->have_posts()) {
			...
			if( $this->profile_fields_stats['fields_all'] > 0 ) {
				...
				$this->profile_fields_stats['completeness_message'] = '<a href="' . $this->user->get_profileurl().'about' .'">' . sprintf(__("Your profile is %d%% complete", 'peepso'), $this->profile_fields_stats['completeness']) .'</a>';
			 } else {
			...
			if( $this->profile_fields_stats['fields_required'] > 0 ) {
				...
				$this->profile_fields_stats['missing_required_message'] = sprintf(__("%d required field(s) missing", 'peepso'), $this->profile_fields_stats['missing_required']);
			} else {
