...
class PeepSoConfigSections extends PeepSoConfigSectionAbstract
{
	...
	private function _group_registration()
	{
		...
		$this->set_field(
			...
			__('Force Profile Completion', 'peepso'),
			'yesno_switch'
		);
		...
		$summary = __('Switching this setting on will force user profile completion. This means, that user will have to fill in all required fields before being able to participate in the community.', 'peepso');
		$this->set_field(
		...
		$this->set_field(
				...
				__('Force SSL on Registration Page', 'peepso'),
				'yesno_switch'
		);

		$this->set_field(
				'site_registration_enable_ssl_description',
				__('Requires a valid SSL certificate. Enabling this option without a valid certificate might break your site.', 'peepso'),
				'message'
		);
		...
		$this->set_field(
				'message_wsl',
				sprintf(__('Requires %s to be installed and properly configured. This is a third party plugin, so use at your own risk.', 'peepso'), $wsl),
				'message'
		);
	...
	private function _group_activity()
	{
		...
		if(count($options) > 1 ) {
			...
			$this->set_field(
				'default_stream',
				__('Default stream content', 'peepso'),
				'select'
			);
		}
		...
		$this->set_field(
			...
			__('Redirect to single post view when post is longer than: [n] characters', 'peepso'),
			'text'
		);
