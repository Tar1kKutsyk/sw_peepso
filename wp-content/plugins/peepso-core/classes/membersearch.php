...
class PeepSoMemberSearch implements PeepSoAjaxCallback
{
	...
	public function search(PeepSoAjaxResponse $resp)
	{
		...
		$is_page = intval($input->post('is_page', 0));
		...
		if ($is_page) {
			...
			$order_by	= $input->post('order_by', $column);
			$order		= $input->post('order', ($order_by == $column ? 'ASC' : NULL));
			...
			// Avatar only
			$peepso_args['avatar_custom'] = (int) $input->post('peepso_avatar', 0);
			if ( 1 !== $peepso_args['avatar_custom'] ) {
				unset( $peepso_args['avatar_custom'] );
			}

			// Gender filter
			$peepso_args['meta_gender'] = strtolower($input->post('peepso_gender', ''));
		...
		$query = stripslashes_deep($input->post('query', ''));
	...
	public function show_member($member)
	{
		...
		echo '<div class="ps-members-item-avatar"><div class="ps-avatar">
				<a href="' . $member->get_profileurl() . '">
					<img alt="' . $member->get_fullname() . '"
					src="' . $member->get_avatar() . '" class="ps-name-tips"></a>' .
				'</div>
			</div>
			<div class="ps-members-item-body">
				<a href="' , $member->get_profileurl(), '" class="ps-members-item-title" title="', $member->get_fullname(), '" alt="', $member->get_fullname(), '">'
					, $online , $member->get_fullname() ,
	...
	{
		echo '<a class="ps-avatar" href="' . $member->get_profileurl() . '" title="' . $member->get_fullname() . '">
				<img alt="' . $member->get_fullname() . '"
				src="' . $member->get_avatar() . '" class="ps-name-tips"></a>';
