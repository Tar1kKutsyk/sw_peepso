...
class PeepSoUser
{
	...
	public static function is_accessible_static($acc, $user_id)
	{
		// if the user is checking their own information -- always return TRUE
		// if user is an admin - always return TRUE
		if (PeepSo::get_user_id() === intval($user_id) || PeepSo::is_admin()) {
			return TRUE;
		}
	...
	private function get_name_parts()
	{
		...
		$this->name_parts = apply_filters('peepso_after_get_display_name', array('name'=>$this->name_parts, 'id'=>$this->get_id()));

		if( isset($this->name_parts['name']) ) {
			$this->name_parts = $this->name_parts['name'];
		}
...
//		$this->id = $temp_id;		// we need to keep the *new* user id
		...
		#	PeepSoMailQueue::add_message($adm_id, $data, __('{sitename} - New User Registration', 'peepso'), 'new_user_registration', 'new_user_registration',0,1);
