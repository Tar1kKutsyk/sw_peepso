...
class PeepSoField
{
	...
	{
		$this->user_id = (NULL != $user_id) ? $user_id : PeepSo::get_user_id();
		$this->_get_wp_user();
		...
		$key = strlen($post->post_name) ? $post->post_name : $id;

		$this->id = $id;
		$this->default_desc = __('Enter data', 'peepso');

		$this->input_args['name'] = 'profile_field_' . $id;
		$this->input_args['id'] = 'profile_field_' . $id;
		...
		if ($this->user_id != PeepSo::get_user_id() && !current_user_can('edit_users')) {
	...
	{
		if( 'array' == $this->data_type ) {

			if( $value_array = json_decode(html_entity_decode($value), TRUE)) {
				$value = $value_array;
			} else {
				$value = array();
			}
		...
		if($this->validate()) {
			return $this->set_user_field($this->key, $this->value);
		}
	...
	public function validate()
	{
		if( count($this->meta->validation) ) {
			...
			if(isset($this->meta->validation->required)) {
	...
	{
		$ret = call_user_func(array($this, $this->prop('meta','method_form')));
	...
	public function render_access()
	{
		...
		if( 1 == $this->meta->user_disable_acc) {
			?>
			<span class="ps-profile-privacy--disabled ps-tooltip" title="<?php _e('Some fields privacy is defined by the administrators', 'peepso'); ?>">
