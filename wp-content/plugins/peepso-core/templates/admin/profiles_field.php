...
$open_pref = get_user_meta(PeepSo::get_user_id(), 'admin_profile_field_open_'.$field->prop('id'),TRUE);
...
// Title of the field
$title = ($field->prop('title')) ? $field->prop('title') : __('no title', 'peepso');

if($field->prop('meta','is_core') > 0) {
	$title_after = sprintf(__('Core %s: ', 'peepso'), ($field->prop('meta', 'is_core') == 1) ? 'WordPress' : 'PeepSo') . ucwords(str_replace('_', ' ', $field->user_meta_key_trim($field->key)));
} else {
	ob_start();
...
$tabs = array(
	'general' 		=> array('id'=>1, 'title'=>__('General','peepso')),
	'appearance' 	=> array('id'=>2, 'title'=>__('Apperance','peepso')),
	'privacy' 		=> array('id'=>3, 'title'=>__('Privacy','peepso')),
	'validation' 	=> array('id'=>4, 'title'=>__('Validation','peepso')),
);
...
<div class="postbox <?php echo $classname;?> ps-postbox--settings no-padd <?php echo $postbox_muted;?>" data-id="<?php echo $field->prop('id');?>">
	...
	<h2 class="hndle <?php echo $classname;?> ps-postbox__title ui-sortable-handle ps-js-handle">
		...
		<div class="ps-postbox__title-editor">
			...
			<button class="button ps-js-btn ps-js-cancel"><?php echo __('Cancel', 'peepso'); ?></button>
			<button class="button button-primary ps-js-btn ps-js-save"><?php echo __('Save', 'peepso'); ?></button>
	...
	<div class="ps-js-field" data-id="<?php echo $field->prop('id');?>" style="<?php echo $open;?>">
		<div class="ps-settings">
			...
			<div id="cpf<?php echo $field->prop('id');?>-tab-1" class="ps-tab__content">
				...
				$action = strtolower(get_class($field).'_admin_general');
				echo "<!--$action-->";
			...
			<div id="cpf<?php echo $field->prop('id');?>-tab-2" class="ps-tab__content" style="display:none">
				...
				if(array_key_exists('privacy', $tabs)) {
					...
					$params = array(
						...
						'field' => $field,
						'label' => __('Default Privacy', 'peepso'),
						'label_after' => '',
					);
					...
					$params = array(
						...
						'field' => $field,
						'label' => __('Disable user privacy setting', 'peepso'),
						'label_after' => '',
					);
